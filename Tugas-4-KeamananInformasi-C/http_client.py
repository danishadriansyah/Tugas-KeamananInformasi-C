import requests
import json
import time
import threading
import base64
import hashlib
import secrets
import os

# DES Tables
IP = [58, 50, 42, 34, 26, 18, 10, 2,
      60, 52, 44, 36, 28, 20, 12, 4,
      62, 54, 46, 38, 30, 22, 14, 6,
      64, 56, 48, 40, 32, 24, 16, 8,
      57, 49, 41, 33, 25, 17, 9, 1,
      59, 51, 43, 35, 27, 19, 11, 3,
      61, 53, 45, 37, 29, 21, 13, 5,
      63, 55, 47, 39, 31, 23, 15, 7]

FP = [40, 8, 48, 16, 56, 24, 64, 32,
      39, 7, 47, 15, 55, 23, 63, 31,
      38, 6, 46, 14, 54, 22, 62, 30,
      37, 5, 45, 13, 53, 21, 61, 29,
      36, 4, 44, 12, 52, 20, 60, 28,
      35, 3, 43, 11, 51, 19, 59, 27,
      34, 2, 42, 10, 50, 18, 58, 26,
      33, 1, 41, 9, 49, 17, 57, 25]

E = [32, 1, 2, 3, 4, 5,
     4, 5, 6, 7, 8, 9,
     8, 9, 10, 11, 12, 13,
     12, 13, 14, 15, 16, 17,
     16, 17, 18, 19, 20, 21,
     20, 21, 22, 23, 24, 25,
     24, 25, 26, 27, 28, 29,
     28, 29, 30, 31, 32, 1]

S_BOX = [
    [[14, 4, 13, 1, 2, 15, 11, 8, 3, 10, 6, 12, 5, 9, 0, 7],
     [0, 15, 7, 4, 14, 2, 13, 1, 10, 6, 12, 11, 9, 5, 3, 8],
     [4, 1, 14, 8, 13, 6, 2, 11, 15, 12, 9, 7, 3, 10, 5, 0],
     [15, 12, 8, 2, 4, 9, 1, 7, 5, 11, 3, 14, 10, 0, 6, 13]],
    
    [[15, 1, 8, 14, 6, 11, 3, 4, 9, 7, 2, 13, 12, 0, 5, 10],
     [3, 13, 4, 7, 15, 2, 8, 14, 12, 0, 1, 10, 6, 9, 11, 5],
     [0, 14, 7, 11, 10, 4, 13, 1, 5, 8, 12, 6, 9, 3, 2, 15],
     [13, 8, 10, 1, 3, 15, 4, 2, 11, 6, 7, 12, 0, 5, 14, 9]],

    [[10, 0, 9, 14, 6, 3, 15, 5, 1, 13, 12, 7, 11, 4, 2, 8],
     [13, 7, 0, 9, 3, 4, 6, 10, 2, 8, 5, 14, 12, 11, 15, 1],
     [13, 6, 4, 9, 8, 15, 3, 0, 11, 1, 2, 12, 5, 10, 14, 7],
     [1, 10, 13, 0, 6, 9, 8, 7, 4, 15, 14, 3, 11, 5, 2, 12]],

    [[7, 13, 14, 3, 0, 6, 9, 10, 1, 2, 8, 5, 11, 12, 4, 15],
     [13, 8, 11, 5, 6, 15, 0, 3, 4, 7, 2, 12, 1, 10, 14, 9],
     [10, 6, 9, 0, 12, 11, 7, 13, 15, 1, 3, 14, 5, 2, 8, 4],
     [3, 15, 0, 6, 10, 1, 13, 8, 9, 4, 5, 11, 12, 7, 2, 14]],

    [[2, 12, 4, 1, 7, 10, 11, 6, 8, 5, 3, 15, 13, 0, 14, 9],
     [14, 11, 2, 12, 4, 7, 13, 1, 5, 0, 15, 10, 3, 9, 8, 6],
     [4, 2, 1, 11, 10, 13, 7, 8, 15, 9, 12, 5, 6, 3, 0, 14],
     [11, 8, 12, 7, 1, 14, 2, 13, 6, 15, 0, 9, 10, 4, 5, 3]],

    [[12, 1, 10, 15, 9, 2, 6, 8, 0, 13, 3, 4, 14, 7, 5, 11],
     [10, 15, 4, 2, 7, 12, 9, 5, 6, 1, 13, 14, 0, 11, 3, 8],
     [9, 14, 15, 5, 2, 8, 12, 3, 7, 0, 4, 10, 1, 13, 11, 6],
     [4, 3, 2, 12, 9, 5, 15, 10, 11, 14, 1, 7, 6, 0, 8, 13]],

    [[4, 11, 2, 14, 15, 0, 8, 13, 3, 12, 9, 7, 5, 10, 6, 1],
     [13, 0, 11, 7, 4, 9, 1, 10, 14, 3, 5, 12, 2, 15, 8, 6],
     [1, 4, 11, 13, 12, 3, 7, 14, 10, 15, 6, 8, 0, 5, 9, 2],
     [6, 11, 13, 8, 1, 4, 10, 7, 9, 5, 0, 15, 14, 2, 3, 12]],

    [[13, 2, 8, 4, 6, 15, 11, 1, 10, 9, 3, 14, 5, 0, 12, 7],
     [1, 15, 13, 8, 10, 3, 7, 4, 12, 5, 6, 11, 0, 14, 9, 2],
     [7, 11, 4, 1, 9, 12, 14, 2, 0, 6, 10, 13, 15, 3, 5, 8],
     [2, 1, 14, 7, 4, 10, 8, 13, 15, 12, 9, 0, 3, 5, 6, 11]]
]

P = [16, 7, 20, 21, 29, 12, 28, 17,
     1, 15, 23, 26, 5, 18, 31, 10,
     2, 8, 24, 14, 32, 27, 3, 9,
     19, 13, 30, 6, 22, 11, 4, 25]

PC1 = [57, 49, 41, 33, 25, 17, 9,
       1, 58, 50, 42, 34, 26, 18,
       10, 2, 59, 51, 43, 35, 27,
       19, 11, 3, 60, 52, 44, 36,
       63, 55, 47, 39, 31, 23, 15,
       7, 62, 54, 46, 38, 30, 22,
       14, 6, 61, 53, 45, 37, 29,
       21, 13, 5, 28, 20, 12, 4]

PC2 = [14, 17, 11, 24, 1, 5,
       3, 28, 15, 6, 21, 10,
       23, 19, 12, 4, 26, 8,
       16, 7, 27, 20, 13, 2,
       41, 52, 31, 37, 47, 55,
       30, 40, 51, 45, 33, 48,
       44, 49, 39, 56, 34, 53,
       46, 42, 50, 36, 29, 32]

SHIFT = [1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1]

SHARED_DES_KEY = None
CLIENT_RSA_PUBLIC_KEY = None
CLIENT_RSA_PRIVATE_KEY = None

# ==================== RSA FUNCTIONS ====================
def gcd(a, b):
    while b:
        a, b = b, a % b
    return a

def mod_inverse(e, phi):
    def extended_gcd(a, b):
        if a == 0:
            return b, 0, 1
        gcd_val, x1, y1 = extended_gcd(b % a, a)
        x = y1 - (b // a) * x1
        y = x1
        return gcd_val, x, y
    
    _, x, _ = extended_gcd(e % phi, phi)
    return (x % phi + phi) % phi

def is_prime(n, k=5):
    if n < 2:
        return False
    if n == 2 or n == 3:
        return True
    if n % 2 == 0:
        return False
    
    r, d = 0, n - 1
    while d % 2 == 0:
        r += 1
        d //= 2
    
    for _ in range(k):
        a = secrets.randbelow(n - 3) + 2
        x = pow(a, d, n)
        
        if x == 1 or x == n - 1:
            continue
        
        for _ in range(r - 1):
            x = pow(x, 2, n)
            if x == n - 1:
                break
        else:
            return False
    return True

def generate_prime(bits=512):
    while True:
        num = secrets.randbits(bits)
        num |= (1 << bits - 1) | 1
        if is_prime(num):
            return num

def generate_rsa_keypair(bits=1024):
    p = generate_prime(bits // 2)
    q = generate_prime(bits // 2)
    
    n = p * q
    phi = (p - 1) * (q - 1)
    
    e = 65537
    while gcd(e, phi) != 1:
        e = secrets.randbelow(phi - 2) + 2
    
    d = mod_inverse(e, phi)
    
    return (e, n), (d, n)

def rsa_encrypt(message, public_key):
    e, n = public_key
    return pow(message, e, n)

def rsa_decrypt(ciphertext, private_key):
    d, n = private_key
    return pow(ciphertext, d, n)

def sign_message(message, private_key):
    """
    Create RSA signature for message using private key
    Returns integer signature
    """
    # Hash the message
    message_hash = hashlib.sha256(message.encode()).digest()
    hash_int = int.from_bytes(message_hash, 'big')
    
    # Sign with private key (encrypt hash)
    d, n = private_key
    signature = pow(hash_int, d, n)
    
    return signature

def perform_key_exchange(server_url, my_name):
    global SHARED_DES_KEY, CLIENT_RSA_PUBLIC_KEY, CLIENT_RSA_PRIVATE_KEY
    
    print("\n=== RSA KEY EXCHANGE ===")
    
    try:
        # Generate client's own RSA keypair for signing
        print("Generating RSA keypair untuk digital signature...")
        CLIENT_RSA_PUBLIC_KEY, CLIENT_RSA_PRIVATE_KEY = generate_rsa_keypair(1024)
        print("RSA keypair generated!")
        
        # Get server's public key
        response = requests.post(server_url, 
            json={'action': 'get_server_public_key'}, 
            timeout=30)
        
        if response.status_code != 200:
            print("Gagal mendapat server public key")
            return False
        
        data = response.json()
        e = int(data['e'])
        n = int(data['n'])
        server_public_key = (e, n)
        
        print("Menerima RSA public key dari server")
        
        # Load or generate DES key
        key_file = f'.client_{my_name}_des_key.bin'
        if os.path.exists(key_file):
            with open(key_file, 'rb') as f:
                SHARED_DES_KEY = f.read()
            print(f"Load DES key dari file (persistent)")
        else:
            # Generate random 8-byte DES key
            SHARED_DES_KEY = secrets.token_bytes(8)
            with open(key_file, 'wb') as f:
                f.write(SHARED_DES_KEY)
            print(f"Generate DES key baru (disimpan)")
        
        print(f"DES Key: {SHARED_DES_KEY.hex()}")
        
        # Convert DES key to integer for RSA encryption
        des_key_int = int.from_bytes(SHARED_DES_KEY, 'big')
        
        # Encrypt DES key with server's RSA public key
        encrypted_des_key = rsa_encrypt(des_key_int, server_public_key)
        
        # Send encrypted DES key AND client's public key to server
        client_e, client_n = CLIENT_RSA_PUBLIC_KEY
        response = requests.post(server_url,
            json={
                'action': 'exchange_key',
                'client_name': my_name,
                'encrypted_des_key': str(encrypted_des_key),
                'client_public_key_e': str(client_e),
                'client_public_key_n': str(client_n)
            },
            timeout=30)
        
        if response.status_code != 200:
            print("Gagal mengirim encrypted DES key")
            return False
        
        print("Key exchange dengan server berhasil")
        print("Public key untuk signature verification dikirim ke server")
        print("="*40 + "\n")
        
        return True
        
    except requests.exceptions.Timeout:
        print("Timeout - Server tidak merespons")
        return False
    except requests.exceptions.ConnectionError:
        print("Koneksi gagal - Cek URL server")
        return False
    except Exception as e:
        print(f"Error: {e}")
        import traceback
        traceback.print_exc()
        return False

# ==================== DES FUNCTIONS ====================
def permute(block, table):
    return ''.join(block[i - 1] for i in table)

def string_to_binary(text):
    if isinstance(text, bytes):
        return ''.join(format(byte, '08b') for byte in text)
    else:
        return ''.join(format(ord(c), '08b') for c in text)

def binary_to_string(binary):
    return ''.join(chr(int(binary[i:i+8], 2)) for i in range(0, len(binary), 8))

def left_shift(bits, n):
    return bits[n:] + bits[:n]

def xor(a, b):
    return ''.join('1' if x != y else '0' for x, y in zip(a, b))

def generate_subkeys(key):
    if isinstance(key, bytes):
        key_bin = string_to_binary(key)
    else:
        key_bin = string_to_binary(key.decode())
    key_pc1 = permute(key_bin, PC1)
    c = key_pc1[:28]
    d = key_pc1[28:]
    subkeys = []
    for i in range(16):
        c = left_shift(c, SHIFT[i])
        d = left_shift(d, SHIFT[i])
        subkey = permute(c + d, PC2)
        subkeys.append(subkey)
    return subkeys

def f_function(right, subkey):
    expanded = permute(right, E)
    xor_result = xor(expanded, subkey)
    s_box_output = ''
    for i in range(8):
        block = xor_result[i*6:(i+1)*6]
        row = int(block[0] + block[5], 2)
        col = int(block[1:5], 2)
        s_box_value = S_BOX[i][row][col]
        s_box_output += format(s_box_value, '04b')
    return permute(s_box_output, P)

def des_round(left, right, subkey):
    new_left = right
    new_right = xor(left, f_function(right, subkey))
    return new_left, new_right

def des_encrypt_block(block, subkeys):
    block = permute(block, IP)
    left = block[:32]
    right = block[32:]
    for i in range(16):
        left, right = des_round(left, right, subkeys[i])
    return permute(right + left, FP)

def des_decrypt_block(block, subkeys):
    block = permute(block, IP)
    left = block[:32]
    right = block[32:]
    for i in range(15, -1, -1):
        left, right = des_round(left, right, subkeys[i])
    return permute(right + left, FP)

def encrypt_message(message):
    subkeys = generate_subkeys(SHARED_DES_KEY)
    padded_message = message.ljust((len(message) + 7) // 8 * 8, '\x00')
    encrypted_blocks = []
    for i in range(0, len(padded_message), 8):
        block = padded_message[i:i+8]
        block_bin = string_to_binary(block)
        encrypted_block = des_encrypt_block(block_bin, subkeys)
        encrypted_blocks.append(binary_to_string(encrypted_block))
    encrypted_message = ''.join(encrypted_blocks)
    return base64.b64encode(encrypted_message.encode('latin-1')).decode('utf-8')

def decrypt_message(encrypted_message):
    subkeys = generate_subkeys(SHARED_DES_KEY)
    encrypted_binary = base64.b64decode(encrypted_message).decode('latin-1')
    decrypted_blocks = []
    for i in range(0, len(encrypted_binary), 8):
        block = encrypted_binary[i:i+8]
        block_bin = string_to_binary(block)
        decrypted_block = des_decrypt_block(block_bin, subkeys)
        decrypted_blocks.append(binary_to_string(decrypted_block))
    decrypted_message = ''.join(decrypted_blocks)
    return decrypted_message.rstrip('\x00')

def listen_for_messages(server_url, my_name):
    last_message_count = 0
    
    while True:
        try:
            response = requests.post(server_url, 
                json={
                    'action': 'get_messages',
                    'client_name': my_name
                }, 
                timeout=30)
            
            if response.status_code == 200:
                data = response.json()
                if data['status'] == 'success' and data['messages']:
                    new_messages = data['messages'][last_message_count:]
                    for msg in new_messages:
                        sender = msg['sender']
                        encrypted_msg = msg['message']
                        signature_verified = msg.get('signature_verified', False)
                        
                        try:
                            decrypted_msg = decrypt_message(encrypted_msg)
                            # Show signature verification status
                            if signature_verified:
                                print(f"\n{sender}: {decrypted_msg} ✓")
                            else:
                                print(f"\n{sender}: {decrypted_msg} (no signature)")
                        except:
                            print(f"\n{sender}: [dekripsi gagal]")
                        print(">>> ", end="", flush=True)
                    
                    last_message_count = len(data['messages'])
            
            time.sleep(2)
            
        except requests.RequestException:
            time.sleep(5)
        except KeyboardInterrupt:
            break

def main():
    global SHARED_DES_KEY
    
    print("="*60)
    print(" SECURE CHAT CLIENT (RSA + DES + DIGITAL SIGNATURE)")
    print("="*60)

    
    server_url = input("\nMasukkan Server URL: ").strip()
    
    # Auto-add https if needed
    if not server_url.startswith('http'):
        if 'localhost' in server_url or '127.0.0.1' in server_url:
            server_url = 'http://' + server_url
        else:
            server_url = 'https://' + server_url
    
    print(f"\nMenghubungi: {server_url}")
    
    # Join chat room
    try:
        response = requests.post(server_url, json={'action': 'join'}, timeout=30)
        if response.status_code == 200:
            data = response.json()
            my_name = data['client_name']
            print(f"Bergabung sebagai: {my_name}")
        else:
            print("Tidak bisa join - Server menolak koneksi")
            return
    except requests.exceptions.Timeout:
        print("Timeout - Server tidak merespons")
        return
    except requests.exceptions.ConnectionError:
        print("Koneksi gagal - Periksa URL dan koneksi internet")
        return
    except Exception as e:
        print(f"Error: {e}")
        return
    
    # Perform key exchange
    if not perform_key_exchange(server_url, my_name):
        print("Key exchange gagal - Chat tidak bisa dilanjutkan")
        return
    
    # Start listening thread
    listen_thread = threading.Thread(target=listen_for_messages, args=(server_url, my_name), daemon=True)
    listen_thread.start()
    
    print("Chat aktif! Ketik 'quit' untuk keluar")
    print("Semua pesan akan ditandatangani secara digital")
    print("="*60 + "\n")
    
    try:
        while True:
            message = input(">>> ")
            
            if message.lower() == 'quit':
                requests.post(server_url, json={'action': 'quit', 'client_name': my_name}, timeout=5)
                print(f"\n{my_name} keluar dari chat")
                break
            
            if not message.strip():
                continue
            
            try:
                # Encrypt message with DES
                encrypted_message = encrypt_message(message)
                
                # Sign the plaintext message with RSA private key
                signature = sign_message(message, CLIENT_RSA_PRIVATE_KEY)
                
                response = requests.post(server_url, 
                    json={
                        'action': 'send_message',
                        'message': encrypted_message,
                        'client_name': my_name,
                        'signature': str(signature)
                    }, 
                    timeout=30)
                
                if response.status_code == 200:
                    result = response.json()
                    if not result.get('success'):
                        if result.get('message'):
                            print(f"⚠️  Server error: {result.get('message')}")
                    elif result.get('signature_verified'):
                        # Signature was verified by server
                        pass
                elif response.status_code == 400:
                    print("Gagal mengirim (key mismatch)")
                    print("Solusi: Hapus file .client_*_des_key.bin dan restart")
                else:
                    print("Gagal mengirim pesan")
                    
            except requests.exceptions.Timeout:
                print("Timeout - Pesan mungkin tidak terkirim")
            except Exception as e:
                print(f"Error: {e}")
                
    except KeyboardInterrupt:
        try:
            requests.post(server_url, json={'action': 'quit', 'client_name': my_name}, timeout=5)
        except:
            pass
        print(f"\n\n{my_name} keluar")

if __name__ == "__main__":
    main()
